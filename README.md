...
- **Chat-driven UI building** — natural language prompts edit React source files via OpenCode
- **Live preview** — Vite HMR shows changes instantly in a split-screen iframe
- **Streaming agent feedback** — every tool call (read, edit, bash) shows up in chat as it happens
- **Multi-project workspaces** — each project gets its own subdirectory; switch between them with one click, code is preserved
- **Element picker** — click any element in the preview to inject its exact source location + selector into your next prompt
...
| `lingua-orchestrator` | Python backend (chat, git, projects, workspace)       |
| `lingua-workspace`    | The sandbox: OpenCode + Vite editing `/project`      |
| `lingua-redis`        | Event streams (real-time chat updates)               |
...
docker compose down -v       # stop + remove everything (projects, redis state, code wiped)
```
...
| -------------------------- | -------- | -------------- | -------------------------------------------------------------------- |
| `OPENROUTER_API_KEY`       | **Yes**  | —              | LLM API key used by OpenCode                                         |
...
The OpenCode behaviour — model, skills, MCP servers, system prompt — lives in a **separate git repo** so it can be updated independently of either Lingua itself or the bootstrap repos. Point `AGENT_CONFIG_REPO_URL` at that repo. See [`docs/refactor/07-agent-config.md`](docs/refactor/07-agent-config.md) for the schema.
...
| Topic | Where |
|-------|-------|
| Architecture, layers, Docker layout | [`docs/refactor/00-architecture.md`](docs/refactor/00-architecture.md) |
| Feature specs (chat, preview, git, projects, picker, …) | [`docs/refactor/`](docs/refactor/) |
